...
description: Fetch a region of cis-eQTL summary statistics from EBI eQTL Catalogue v7+ via tabix-on-FTP. Use when an agent needs eQTL beta / SE / p-value for every variant in a window around a gene's TSS for one specific dataset (study × tissue × quantification method). Input:dataset_id, chromosome, start, end, optional molecular_trait_id. Output:harmonised TSV slice.
license: "MIT"
metadata: {"demo_data":["examples/input.json"],"dependencies":["python\u003e=3.10","pysam\u003e=0.22","pandas\u003e=2.0","requests\u003e=2.28"],"domain":"bioinformatics","endpoints":["https://ftp.ebi.ac.uk/pub/databases/spot/eQTL/sumstats/","https://www.ebi.ac.uk/eqtl/api/v3/"],"inputs":[{"description":"eQTL Catalogue dataset identifier (e.g. QTD000276 for GTEx minor salivary gland ge-eQTL).","name":"dataset_id","required":true,"type":"string"},{"description":"Chromosome name without `chr` prefix (1, 2, ..., X, Y, MT).","name":"chromosome","required":true,"type":"string"},{"description":"Region start, 1-based GRCh38.","name":"start_bp","required":true,"type":"integer"},{"description":"Region end, 1-based GRCh38 (inclusive).","name":"end_bp","required":true,"type":"integer"},{"description":"Optional ENSG (versioned or bare) to filter to one gene; required for ge-eQTL datasets where one TSV bundles multiple traits.","name":"molecular_trait_id","required":false,"type":"string"}],"openclaw":{"always":false,"emoji":"🧬","homepage":"https://github.com/ClawBio/ClawBio","install":"pip install pysam pandas requests\n","os":["darwin","linux"],"requires":{"bins":["python3","tabix"],"config":null,"env":null},"trigger_keywords":["eqtl region fetch","eqtl catalogue tabix","eqtl sumstats slice","cis-eqtl region pull","GTEx eqtl region"]},"outputs":[{"description":"Per-variant rows with variant_id, chromosome, position, ref, alt, beta, se, p_value, maf, molecular_trait_id, dataset_id.","name":"variants","type":"list"},{"description":"EQTLCatalogueRelease with study_label, tissue_label, condition_label, sample_group, quant_method, dataset_release, fetched_at_utc.","name":"release","type":"object"}],"skill-author":"Aviv Madar","tags":["eqtl","eqtl-catalogue","region-fetch","tabix","summary-statistics","cis-eqtl"],"version":"0.1.0"}
---
