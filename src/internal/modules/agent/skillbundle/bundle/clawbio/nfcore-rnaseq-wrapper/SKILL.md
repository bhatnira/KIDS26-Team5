...
license: "MIT"
metadata: {"author":"ClawBio","demo_data":[{"description":"Demo mode uses the upstream nf-core/rnaseq test profile rather than bundled FASTQs","path":"demo/README.md"}],"dependencies":{"packages":null,"python":"\u003e=3.10"},"domain":"transcriptomics","endpoints":{"cli":"python clawbio.py run rnaseq-pipeline --input {samplesheet} --output {output_dir}"},"inputs":[{"description":"nf-core/rnaseq samplesheet. Required columns: sample, fastq_1, strandedness. FASTQ mode may add fastq_2. BAM reprocessing mode preserves the original FASTQ columns and adds genome_bam and/or transcriptome_bam plus percent_mapped; use it only with --skip-alignment. Optional metadata columns: seq_platform, seq_center.\n","format":["csv"],"name":"samplesheet","required":false,"type":"file"}],"openclaw":{"always":false,"emoji":"🧬","homepage":"https://github.com/ClawBio/ClawBio","install":null,"os":["darwin","linux"],"requires":{"bins":["python3","nextflow","java"],"config":null,"env":null},"trigger_keywords":["bulk RNA-seq preprocessing","nf-core rnaseq","run rnaseq from fastq","preprocess RNA-seq FASTQs","FASTQ to count matrix","STAR Salmon RNA-seq pipeline","RSEM RNA-seq pipeline","HISAT2 RNA-seq alignment","bowtie2 salmon prokaryotic rnaseq"]},"outputs":[{"description":"Wrapper run summary and downstream handoff recommendations","format":["md"],"name":"report","type":"file"},{"description":"Structured result payload with detected count matrices and provenance","format":["json"],"name":"result","type":"file"}],"tags":["rnaseq","bulk-rna-seq","nextflow","nf-core","fastq","preprocessing","counts"],"version":"0.1.0"}
---
