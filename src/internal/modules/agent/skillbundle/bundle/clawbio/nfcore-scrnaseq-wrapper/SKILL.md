...
license: "MIT"
metadata: {"author":"ClawBio","demo_data":[{"description":"Demo mode uses the upstream nf-core/scrnaseq -profile test dataset rather than bundled FASTQs","path":"demo/README.md"}],"dependencies":{"packages":["pyyaml"],"python":"\u003e=3.11"},"domain":"transcriptomics","endpoints":{"cli":"python clawbio.py run scrnaseq-pipeline --input {samplesheet} --output {output_dir}"},"inputs":[{"description":"nf-core/scrnaseq samplesheet CSV with required columns: sample, fastq_1, fastq_2","format":["csv"],"name":"samplesheet","required":true,"type":"file"}],"openclaw":{"always":false,"emoji":"🧫","homepage":"https://github.com/ClawBio/ClawBio","os":["darwin","linux"],"requires":{"bins":["python3","nextflow","java"]},"trigger_keywords":["scrnaseq","nf-core scrnaseq","run scrnaseq from fastq","preprocess 10x fastqs","generate h5ad from single-cell fastq","single-cell preprocessing","nextflow scrna pipeline","10x chromium fastq pipeline","starsolo upstream processing","alevin-fry fastq to counts","run nextflow scrnaseq","upstream single-cell pipeline","fastq to h5ad single cell","10x genomics fastq pipeline"]},"outputs":[{"description":"Wrapper run summary and downstream handoff recommendations","format":["md"],"name":"report","type":"file"},{"description":"Structured result payload with detected outputs and provenance","format":["json"],"name":"result","type":"file"}],"tags":["scrna","single-cell","nextflow","nf-core","fastq","10x","h5ad","preprocessing"],"version":"0.1.0"}
---
...
- **The `validation.*` / `validationSchemaIgnoreParams` warnings on a run or a bundle replay come from the pipeline, not from the wrapper. Do NOT try to silence them by adding params.** nf-core/scrnaseq 4.1.0's own `nextflow.config` declares `plugins { id 'nf-schema@2.5.1' }` and a matching `validation { defaultIgnoreParams = ["genomes"]; monochromeLogs = params.monochrome_logs }` scope, and it carries a `monochromeLogs = null` param that upstream itself annotates `// TODO temporary workaround a warning` (with a matching ignored lint check in its `.nf-core.yml`). nf-schema surfaces that as `WARN: The following invalid input values have been detected: * --validationSchemaIgnoreParams: genomes`. The wrapper writes **none** of these keys — its `params.yaml` carries only the audited CLI surface — and the bundle's `commands.sh` runs the pinned pipeline unmodified, so the same warning appears on a plain upstream `nextflow run nf-core/scrnaseq -r 4.1.0` too. It is harmless upstream noise: the plugin loads correctly (`Plugins declared=[nf-schema@2.5.1]`) and validation still runs. Writing `validation`/`monochromeLogs` into `params.yaml` to quiet it would override the pipeline's own config and is exactly the wrong fix — the wrapper's "omit = trust upstream default" rule applies. A regression test pins that these keys never enter `params.yaml`.
