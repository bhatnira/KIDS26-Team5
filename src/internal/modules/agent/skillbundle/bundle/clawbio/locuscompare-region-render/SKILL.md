...
description: Render a 4-panel regional LocusCompare diagnostic for one (lead variant, exposure study, outcome study) tuple - overlays GWAS Manhattan, QTL Manhattan, GENCODE gene track, and cross-trait scatter colored by LD r². Use when an agent needs visual confirmation that two GWAS / QTL signals share the same causal variant (the Liu 2019 LocusCompare convention). Inputs:lead variant + two pre-fetched harmonised sumstats slices (or eQTL Catalogue / GWAS Catalog identifiers for bundled fetch). Output:PNG + JSON manifest.
license: "MIT"
metadata: {"demo_data":["examples/01_synthetic_demo/config.json","examples/02_eqtl_catalogue_x_gwas_catalog/config.yaml"],"dependencies":["python\u003e=3.10","numpy\u003e=1.24","scipy\u003e=1.10","pandas\u003e=2.0","matplotlib\u003e=3.7","pysam\u003e=0.22","pyyaml\u003e=6.0","pydantic\u003e=2.0","requests\u003e=2.28"],"domain":"bioinformatics","endpoints":["https://ftp.ebi.ac.uk/pub/databases/spot/eQTL/sumstats/","https://ftp.ebi.ac.uk/pub/databases/gwas/summary_statistics/","https://ftp.1000genomes.ebi.ac.uk/","https://rest.ensembl.org/"],"inputs":[{"description":"Run config (JSON or YAML) describing the lead variant + region,\nexposure and outcome data sources (either pre-fetched harmonised TSV\npaths OR a bundled-fetcher block referencing eQTL Catalogue / GWAS\nCatalog), and optional LD + gene-track sources. See INPUT_SCHEMA.md\nfor the canonical sumstats-slice TSV column spec.\n","name":"input_file","required":true,"type":"file"}],"openclaw":{"always":false,"emoji":"🎯","homepage":"https://github.com/ClawBio/ClawBio","install":"pip install 'numpy\u003e=1.24' 'scipy\u003e=1.10' 'pandas\u003e=2.0' 'matplotlib\u003e=3.7' \\\n            'pysam\u003e=0.22' 'pyyaml\u003e=6.0' 'pydantic\u003e=2.0' 'requests\u003e=2.28'\n# plus a system plink 1.9 binary (brew install plink on macOS, apt-get install plink1.9 on Linux)\n# Pins mirror the `dependencies:` block above; keep both in sync.\n","os":["darwin","linux"],"requires":{"bins":["python3","tabix","plink"],"config":null,"env":null},"trigger_keywords":["regional locuscompare","regional coloc plot","two-trait coloc visualization","GWAS eQTL overlay","liu 2019 locuscompare convention","colocalization plot for a lead variant","regional plot for coloc lead"]},"outputs":[{"description":"Markdown report describing the rendered plot + caveats","name":"report","type":"file"},{"description":"4-panel regional LocusCompare PNG (exposure Manhattan + outcome Manhattan + gene track + cross-trait scatter)","name":"figure","type":"file"},{"description":"Reproducibility manifest (YAML) with source releases, LD panel id, plink version, n_pairs, palindromic-exclusion count","name":"manifest","type":"file"}],"skill-author":"Aviv Madar","tags":["regional-plot","coloc","eqtl","gwas","locuscompare","mendelian-randomisation","target-validation","drug-discovery"],"version":"0.1.0"}
---
