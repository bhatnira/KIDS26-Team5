---
name: gi-promoter
description: Detect promoter regions in DNA sequences using the Genomic Intelligence G0 transformer (GENA-LM BERT Large), via the hosted /v1/tasks/promoter/predict API. Returns per-window promoter probabilities and called regions.
license: "MIT"
metadata: {"author":"ClawBio + Genomic Intelligence","demo_data":[{"description":"TP53 locus, gene-sense (chr17:7661779-7687546, GRCh38, 25.8 kbp; TP53 is minus-strand, so this is the reverse complement) — bundled real reference sequence.","path":"example_data/promoter_tp53.fa"}],"dependencies":{"packages":["requests\u003e=2.31"],"python":"\u003e=3.10"},"domain":"genomics","endpoints":{"cli":"python skills/gi-promoter/gi_promoter.py --input {input_file} --output {output_dir}"},"inputs":[{"description":"Single-record FASTA. Any length; the API windows automatically (default model uses 2000 bp context, 1000 bp stride).","format":["fa","fasta","fna"],"name":"input_file","required":false,"type":"file"}],"openclaw":{"always":false,"emoji":"🧬","homepage":"https://docs.genomicintelligence.ai","install":[{"bins":null,"kind":"pip","package":"requests"}],"os":["darwin","linux"],"requires":{"bins":["python3"],"config":null,"env":null},"trigger_keywords":["promoter","promoter prediction","predict promoter","find promoter","promoter region","TSS prediction","transcription start site","gi promoter","genomic intelligence promoter","G0 promoter","GENA-LM promoter"]},"outputs":[{"description":"Markdown report — sequence + model metadata, called promoter regions, headline counts.","format":"md","name":"report","type":"file"},{"description":"Full `{data, meta}` response from the GI API plus a flattened summary.","format":"json","name":"result","type":"file"},{"description":"command.sh + environment.json for exact-rerun reproducibility.","name":"reproducibility","type":"directory"}],"tags":["genomics","promoter","transcription","regulatory","dna-lm","transformer","gi-api"],"version":"0.1.0"}
---
