...
description: Compute pairwise r² between a lead variant and every variant in a window using the 1000 Genomes Phase 3 GRCh38 reference panel, ancestry-stratified. Use when an agent needs LD coloring for a regional plot or LD pruning around a candidate causal variant. Single client (on-demand region fetch from EBI 1000G FTP); no multi-GB cold-start.
license: "MIT"
metadata: {"demo_data":["examples/input.json"],"dependencies":["python\u003e=3.10","pysam\u003e=0.22","pandas\u003e=2.0","requests\u003e=2.28"],"domain":"bioinformatics","endpoints":["https://ftp.1000genomes.ebi.ac.uk/"],"inputs":[{"description":"Lead variant id in chr_pos_ref_alt format (e.g. 1_109274968_G_T).","name":"lead","required":true,"type":"string"},{"description":"Iterable of partner variant ids in the same format.","name":"partners","required":true,"type":"list"},{"description":"Chromosome (with or without `chr` prefix).","name":"chromosome","required":true,"type":"string"},{"description":"Total window width around the lead.","name":"window_bp","required":true,"type":"integer"},{"description":"1000G Phase 3 super-population code (EUR / AFR / AMR / EAS / SAS).","name":"super_pop","required":false,"type":"string"}],"openclaw":{"always":false,"emoji":"🧬","homepage":"https://github.com/ClawBio/ClawBio","install":"pip install pysam pandas requests\n# plus the plink 1.9 binary (cog-genomics.org/plink/1.9):\n#   macOS (brewsci tap): brew install brewsci/bio/plink\n#   Ubuntu / Debian:     apt-get install plink1.9\n#   any platform:        conda install -c bioconda plink\n#   direct binary:       https://www.cog-genomics.org/plink/1.9/  (then set PLINK_BIN)\n","os":["darwin","linux"],"requires":{"bins":["python3","plink","tabix"],"config":null,"env":null},"trigger_keywords":["ld around lead","ld region 1000g","r-squared 1000g phase 3","ancestry-stratified ld","locuszoom ld coloring","1000 genomes ld panel"]},"outputs":[{"description":"Per-partner LDPair (partner_variant_id, r2, optional dprime).","name":"pairs","type":"list"},{"description":"Panel id, version, plink version, n_partners_returned, fetched_at_utc.","name":"panel_meta","type":"object"}],"skill-author":"Aviv Madar","tags":["ld","1000-genomes","reference-panel","plink","ancestry-stratified","on-demand"],"version":"0.1.0"}
---
