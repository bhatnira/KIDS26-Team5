...
license: "MIT"
metadata: {"author":"Duvet05 \u003cgonzalo.galvezc@pucp.edu.pe\u003e","demo_data":[{"description":"Synthetic query for BRCA1 breast cancer trials — exercises recruiting and completed status paths","path":"demo_input.txt"}],"dependencies":{"python":"\u003e=3.11"},"domain":"clinical","endpoints":{"cli_demo":"python skills/clinical-trial-finder/clinical_trial_finder.py --demo --output {output_dir}","cli_file":"python skills/clinical-trial-finder/clinical_trial_finder.py --input {input_file} --output {output_dir}","cli_gene":"python skills/clinical-trial-finder/clinical_trial_finder.py --gene {gene} --output {output_dir}","cli_query":"python skills/clinical-trial-finder/clinical_trial_finder.py --query \"{query}\" --output {output_dir}","cli_rsid":"python skills/clinical-trial-finder/clinical_trial_finder.py --rsid {rsid} --output {output_dir}"},"inputs":[{"description":"Text file with one search term per line (gene name, condition, or drug)","format":["txt"],"name":"input_file","required":false,"type":"file"},{"description":"Direct search query string (alternative to input_file)","name":"query","required":false,"type":"string"},{"description":"Gene symbol (e.g. BRCA1) — enriched via OpenTargets gene-to-disease mapping","name":"gene","required":false,"type":"string"},{"description":"dbSNP rsID (e.g. rs3798220) — resolved via GWAS Catalog to disease traits, then queried against CT.gov","name":"rsid","required":false,"type":"string"},{"description":"Run with built-in demo data (BRCA1 breast cancer)","name":"demo","required":false,"type":"flag"}],"openclaw":{"always":false,"emoji":"🏥","homepage":"https://github.com/ClawBio/ClawBio","os":["darwin","linux"],"requires":{"bins":["python3"]},"trigger_keywords":["clinical trial","clinical trials","trial finder","FHIR","ResearchStudy","NCT","ClinicalTrials.gov","GWAS","rsID","variant"]},"outputs":[{"description":"Markdown report with matched trials, status, phase, and links to ClinicalTrials.gov","format":"md","name":"report","type":"file"},{"description":"Machine-readable trial data for programmatic use","format":"json","name":"summary","type":"file"},{"description":"FHIR R4 Bundle of ResearchStudy resources with MeSH-coded conditions (written with --fhir)","format":"json","name":"fhir_bundle","type":"file"},{"description":"Stacked bar chart of trial counts by phase, coloured by recruitment status (figures/phase_distribution.png)","format":"png","name":"phase_distribution","type":"file"},{"description":"Exact CLI invocation to reproduce this run (commands.sh)","format":"sh","name":"commands","type":"file"},{"description":"SHA-256 digests of all generated outputs (checksums.sha256)","format":"txt","name":"checksums","type":"file"},{"description":"Interactive HTML report with JS filters (status, phase, text search) and coloured trial cards (report.html)","format":"html","name":"html_report","type":"file"},{"description":"Trial data as CSV for import into Excel, R, or pandas (tables/trials.csv)","format":"csv","name":"csv_table","type":"file"}],"tags":["clinical-trials","genomics","drug-discovery","clinical","precision-medicine"],"version":"0.1.0"}
---
