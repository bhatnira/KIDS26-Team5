---
name: gi-expression
description: Predict tissue / cell-type expression (log TPM + TPM) from a 9,198 bp TSS-centered DNA sequence using the Genomic Intelligence G0 Expression model, via the hosted /v1/tasks/expression/predict API. The model is conditioned on a free-text cell-type / assay description.
license: "MIT"
metadata: {"author":"ClawBio + Genomic Intelligence","demo_data":[{"description":"HBB (β-globin) TSS-centered 9,198 bp window, reverse-complemented to gene-sense. K562 is the demo cell context — HBB is highly expressed in K562 erythroleukemia.","path":"example_data/expression_hbb_k562.fa"}],"dependencies":{"packages":["requests\u003e=2.31"],"python":"\u003e=3.10"},"domain":"genomics","endpoints":{"cli":"python skills/gi-expression/gi_expression.py --input {input_file} --output {output_dir}"},"inputs":[{"description":"Single-record FASTA. The expression model expects exactly 9,198 bp centered on the TSS, gene-sense (RC minus-strand genes).","format":["fa","fasta","fna"],"name":"input_file","required":false,"type":"file"}],"openclaw":{"always":false,"emoji":"🧪","homepage":"https://docs.genomicintelligence.ai","install":[{"bins":null,"kind":"pip","package":"requests"}],"os":["darwin","linux"],"requires":{"bins":["python3"],"config":null,"env":null},"trigger_keywords":["expression prediction","predict expression","sequence to expression","TPM prediction","cell type expression","tissue expression","RNA-seq prediction","gi expression","G0 expression","genomic intelligence expression"]},"outputs":[{"description":"Markdown report — predicted log(TPM+1), TPM, model + timing.","format":"md","name":"report","type":"file"},{"description":"Full `{data, meta}` response.","format":"json","name":"result","type":"file"},{"description":"command.sh + environment.json.","name":"reproducibility","type":"directory"}],"tags":["genomics","expression","RNA-seq","TPM","sequence-to-expression","dna-lm","gi-api"],"version":"0.1.0"}
---
