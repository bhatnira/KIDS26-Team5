...
description: Fetch a regional slice of plasma pQTL summary statistics from the UK Biobank Pharma Proteomics Project (UKB-PPP; Sun 2023 Nature) for a specific (protein, ancestry) measurement. Use when an agent needs per-variant beta / SE / p-value around a coloc-lead variant for downstream colocalisation, Mendelian randomisation, or regional plotting against a pQTL exposure. The canonical use case is the cis-window around the protein's coding gene TSS, but UKB-PPP releases full-genome summary stats per protein so any GRCh38 window (including trans loci) is supported when the user supplies an explicit (chromosome, start_bp, end_bp). Input:protein_label (HGNC or UniProt), ancestry, chromosome, start_bp, end_bp. Output:harmonised TSV slice + manifest + human-readable report.
license: "MIT"
metadata: {"demo_data":["examples/sort1_ukb_ppp_eur.json"],"dependencies":["python\u003e=3.10","synapseclient\u003e=3.0","requests\u003e=2.28"],"domain":"bioinformatics","endpoints":["https://repo-prod.prod.sagebase.org/repo/v1/","s3://ukbiobank.opendata.sagebase.org"],"env":[{"description":"Free Synapse personal access token. Required for the file-download path; not required for the listing-only smoke test. Obtain via https://www.synapse.org/Profile:settings (\"Personal Access Tokens\"). No UKB Application required for the summary-stats layer.","name":"SYNAPSE_AUTH_TOKEN","required":true}],"inputs":[{"description":"HGNC symbol (e.g. SORT1) or UniProt accession (e.g. Q99523). Resolves to the canonical UKB-PPP per-protein file via the Synapse listing.","name":"protein_label","required":true,"type":"string"},{"description":"One of EUR (European discovery; N=46,673), AFR (African; N=931), CSA / SAS (Central / South Asian; N=920), EAS (East Asian; N=262), MID (Middle East; N=124), AMR (American Hispanic; N=60), ALL (Combined multi-ancestry meta; N=47,970). Per Sun 2023 Nature Table 1.","name":"ancestry","required":true,"type":"string"},{"description":"Chromosome without `chr` prefix (1, 2, ..., X). UKB-PPP per-protein archive has one file per chromosome.","name":"chromosome","required":true,"type":"string"},{"description":"Region start, 1-based GRCh38.","name":"start_bp","required":true,"type":"integer"},{"description":"Region end, 1-based GRCh38 (inclusive).","name":"end_bp","required":true,"type":"integer"}],"openclaw":{"always":false,"emoji":"🧪","homepage":"https://github.com/ClawBio/ClawBio","install":"pip install synapseclient requests\n","os":["darwin","linux"],"requires":{"bins":["python3"],"config":null,"env":["SYNAPSE_AUTH_TOKEN"]},"trigger_keywords":["ukb-ppp pqtl region fetch","pqtl regional summary stats","sun 2023 ukbppp","protein qtl summary stats","plasma pqtl regional fetch"]},"outputs":[{"description":"Per-variant rows with variant_id (chr_pos_ref_alt, OT convention), chromosome, position, ref, alt, beta, se, p_value (linear; converted from LOG10P), maf, effect_allele_frequency, molecular_trait_id (Olink reagent id), study_id (Synapse fileID).","name":"variants","type":"list"},{"description":"UKBPPPRelease with protein_hgnc, protein_uniprot, olink_reagent_id, olink_panel, ancestry, ancestry_label, n_samples, synapse_id, source_url, release_label, fetched_at_utc.","name":"release","type":"object"}],"skill-author":"Aviv Madar","tags":["pqtl","ukb-ppp","region-fetch","regenie","summary-statistics","proteomics","olink"],"version":"0.1.0"}
---
