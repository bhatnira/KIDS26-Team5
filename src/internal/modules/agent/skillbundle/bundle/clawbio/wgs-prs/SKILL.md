---
name: wgs-prs
description: End-to-end WGS to polygenic risk score pipeline. Takes paired-end FASTQ files (or a pre-existing VCF) through nf-core/sarek for variant calling, applies VCF QC (normalisation, hard filtering, Ti/Tv and Het/Hom checks), then computes polygenic risk scores via the PGS Catalog. Fills the FASTQ to VCF gap upstream of the gwas-prs skill.
license: "MIT"
metadata: {"author":"David de Lorenzo","inputs":[{"description":"Forward reads FASTQ.gz (paired-end WGS)","format":"fastq.gz","name":"fastq_r1","required":false,"type":"file"},{"description":"Reverse reads FASTQ.gz","format":"fastq.gz","name":"fastq_r2","required":false,"type":"file"},{"description":"Pre-existing VCF, skips sarek, starts at QC stage","format":"vcf.gz","name":"input_vcf","required":false,"type":"file"},{"default":"SAMPLE","description":"Sample identifier used throughout the pipeline","name":"sample_id","required":false,"type":"string"},{"default":"XX","description":"Biological sex: XX or XY (affects sex-chromosome calling)","name":"sex","required":false,"type":"string"}],"openclaw":{"always":false,"emoji":"🧬","homepage":"https://github.com/ClawBio/ClawBio","install":[{"bins":["nextflow"],"kind":"url","note":"curl -s https://get.nextflow.io | bash","url":"https://get.nextflow.io"},{"bins":["bcftools"],"kind":"conda","note":"conda install -c bioconda bcftools  (optional, enables full VCF QC)","package":"bcftools"}],"os":["darwin","linux"],"requires":{"anyBins":["docker","singularity"],"bins":["python3","nextflow"],"config":null,"env":null},"trigger_keywords":["WGS","whole genome sequencing","FASTQ to PRS","variant calling","nf-core sarek","sarek","FASTQ VCF","WGS polygenic risk","germline variant calling","GATK HaplotypeCaller","WGS pipeline","raw sequencing to risk scores"]},"outputs":[{"description":"Human-readable summary of all pipeline stages","name":"bridge_report.md"},{"description":"Machine-readable stage status and QC metrics","name":"bridge_report.json"},{"description":"Ti/Tv ratio, Het/Hom ratio, variant counts, pass/fail","name":"vcf_qc/qc_metrics.json"},{"description":"Normalised, filtered canonical VCF ready for PRS scoring","name":"vcf_qc/canonical_pass.vcf.gz"},{"description":"PRS narrative report from gwas-prs","name":"prs_output/report.md"},{"description":"Per-trait PRS scores, percentiles, and risk categories","name":"prs_output/tables/scores.csv"}],"tags":["wgs","whole-genome-sequencing","polygenic-risk-scores","prs","sarek","nf-core","nextflow","variant-calling","vcf-qc","gatk"],"version":"0.1.0"}
---
