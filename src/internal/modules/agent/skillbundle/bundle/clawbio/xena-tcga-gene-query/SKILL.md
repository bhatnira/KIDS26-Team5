...
description: Query TCGA tumor biology through the ucscxenatoolspy API. Supports tumor-vs-normal differential expression, gene-gene correlation, survival association, and cancer catalogue browsing across 30+ TCGA cancer types.
license: "MIT"
metadata: {"author":"lishensuo","demo_data":[{"description":"Example output from running diff-expr, corr, and survival queries via demo mode","path":"examples/demo_output.md"}],"dependencies":{"python":"\u003e=3.10"},"domain":"cancer-genomics","endpoints":{"cli":"python skills/xena-tcga-gene-query/scripts/query_tcga_api.py --task {task} --gene {gene} --cancer {cancer} --output {output_dir}"},"inputs":[{"description":"HGNC gene symbol (e.g. TP53, EGFR, BRCA1). Aliases are resolved by the API.","format":["text"],"name":"gene","required":true,"type":"string"},{"description":"TCGA cancer abbreviation (e.g. LUAD, BRCA, GBM). See references/tcga_codes.md for the complete mapping.","format":["text"],"name":"cancer","required":false,"type":"string"}],"openclaw":{"always":false,"emoji":"🦀","homepage":"https://github.com/lishensuo/UCSCXenaToolsPy","install":[{"kind":"pip","package":""}],"os":["darwin","linux"],"requires":{"bins":["python3"]},"trigger_keywords":["TCGA","tumor vs normal","differential expression","gene expression cancer","survival analysis","co-expression","prognosis","cancer biomarker","肿瘤","肺癌","乳腺癌","差异表达","生存分析","预后"]},"outputs":[{"description":"Markdown report with query results, sample sizes, effect sizes, and interpretation notes","format":["md"],"name":"report","type":"file"},{"description":"Machine-readable API response for downstream chaining","format":["json"],"name":"result","type":"file"},{"description":"Directory with commands.sh and run.json describing the exact API calls","name":"reproducibility","type":"directory"}],"tags":["tcga","gene-expression","survival-analysis","differential-expression","co-expression","cancer","tumor-biology"],"version":"0.1.0"}
---
