...
depends_on: ["machine-learning/biomarker-discovery","machine-learning/model-validation","machine-learning/omics-classifiers","machine-learning/prediction-explanation"]
primary_tool: "sklearn"
qc_checkpoints: [{"after_selection":"Selected features 5-200, stability index reported alongside count"},{"after_cv":"Selection inside the CV pipeline; AUC reported with fold spread; AUPRC/MCC if imbalanced"},{"after_interpretation":"SHAP used as a shortcut/batch audit, aggregated over modules, not as the validated panel"},{"after_validation":"Hold-out AUC with bootstrap CI plus calibration (Brier); external cohort for the real bar"}]
tool_type: "python"
workflow: true
---
