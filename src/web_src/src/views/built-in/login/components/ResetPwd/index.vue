<script setup>
import { fetchResetPassword, fetchResetCode } from '@/api/login'
...
async function handleSendCode() {
  ...
  try {
    ...
    const res = await fetchResetCode({ email: formValue.value.email }).send()
    if (res.isSuccess) {
...
async function handleReset() {
  formRef.value?.validate(async (errors) => {
    ...
    try {
      ...
      await fetchResetPassword({ email, password: pwd, code }).send()
    ...
    <n-form ref="formRef" :rules="rules" :model="formValue" :show-label="false" size="large">
      ...
      <n-form-item>
        <n-space vertical :size="20" class="w-full">
          <n-button block type="primary" @click="handleReset">
