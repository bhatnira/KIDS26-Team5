...
export function useTabScroll(currentTabPath) {
  const scrollbar = ref()
  const safeArea = ref(150)

  const handleTabSwitch = (distance) => {
  ...
  const scrollToCurrentTab = () => {
    nextTick(() => {
      const currentTabElement = document.querySelector(`[data-tab-path="${currentTabPath.value}"]`)
  ...
  const handleScroll = throttle({ interval: 120 }, (step) => {
